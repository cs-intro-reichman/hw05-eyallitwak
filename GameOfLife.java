...
public class GameOfLife {
	...
	private static void test2(String fileName) {
		...
		for (int i = 1; i < board.length - 1; i++) {
			for (int j = 1; j < board[0].length - 1; j++) {
				System.out.printf("%3s", "next gen: " + cellValue(board, i, j) + " ");
			}
			System.out.println();
		}
	}
	...
	public static int cellValue(int[][] board, int i, int j) {
		int neighbors = count(board, i, j), nextGenValue = -1;
		if (board[i][j] == 1) {
			nextGenValue = (neighbors == 2 || neighbors == 3) ? board[i][j] : 0;
		} else {
			nextGenValue = neighbors == 3 ? 1 : board[i][j];
		}
		return nextGenValue;
	}
	...
	public static void show(int[][] board) {
		...
		// scheme used by
		// StdDraw, the RGB codes of black and white are, respectively, (0,0,0) and
		// (255,255,255).
	...
	public static void showEyal(int[][] board) {
